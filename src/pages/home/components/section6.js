import React from 'react'
import { Row, Col, Typography } from 'antd'
import GoogleMapReact from 'google-map-react'
import point from 'assets/point.png'
import './style.css'
const AnyReactComponent = ({ text }) => <div className='pointer-holder'><img className='pointer' src={point} alt='point' /> <br /> <b> {text} </b> </div>;

const { Title } = Typography;
function Section6() {
  const data = {
    center: {
      lat: 21.028874614904453,
      lng: 105.82481571028652
    },
    zoom: 15
  }
  return (
    <section className='section-6'>
      <section className='trans-6'>
        <br />
        <Row>
          <Col xs={24}>
            <Title underline level={2}> Liên hệ </Title>
          </Col>
        </Row>
        <Row>
          <Col xs={24} md={{ span: 16, offset: 4 }}>
              <div class="mapouter">
                <div class="gmap_canvas">
                  <iframe
                    title='ggmap'
                    src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d5473.4312217937295!2d105.81875634683443!3d21.028871598083633!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3135ab76ab560c29%3A0x253637caebea1353!2zQ-G7pWMgUXXhuqNuIEzDvSBExrDhu6Nj!5e0!3m2!1sen!2sus!4v1625208074683!5m2!1sen!2sus"
                    height="450"
                    width="100%"
                    allowfullscreen=""
                    loading="lazy"></iframe>
                  </div>
                </div>
          </Col>
        </Row>
      </section>
    </section>
  )
}
export default Section6
